...
def get_process_tracker():
    tracker = _GLOBALS.get('process.tracker', None)
    if tracker:
        return tracker

    tracker = ProcessTracker()
    _GLOBALS['process.tracker'] = tracker
    return tracker

class ProcessSpawner(object):

    def __init__(self, registry=None, output_dir=None):
        self.tracker = get_process_tracker()
    ...
    def spawn(self, command):
        ...
        with open(outlog, 'wb') as out, open(errlog, 'wb') as err:
            p = Subprocess(command.split(), stdout=out, stderr=err)
...
class ProcessList(tornado.web.RequestHandler):
    def get(self):
        ...
        self.write(json.dumps(tracker.all()))
